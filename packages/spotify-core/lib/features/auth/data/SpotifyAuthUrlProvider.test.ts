...
describe('SpotifyAuthProvider.ts', () => {
  it('should throw Error when clientId is not provided', () => {
    const spotifyAuthProvider = SpotifyAuthUrlProvider.create()
      .setRedirectUri('http%3A%2F%2Flocalhost%3A3000%')
  ...
  test('proper auth url creation', () => {
    const expectedUrl = 'https://accounts.spotify.com/authorize?client_id=06006394f03e41b9af557e5e00ab2220&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Fcallback&scope=user-library-read%2Cstreaming&response_type=token'
    const spotifyAuthProvider = SpotifyAuthUrlProvider.create()
